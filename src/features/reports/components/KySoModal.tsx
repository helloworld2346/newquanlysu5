import { ShieldCheck, PenLine, CalendarClock, User, Award, Briefcase, Building2 } from "lucide-react";  
import {  
  Dialog,  
  DialogContent,  
  DialogHeader,  
      ...
      <DialogContent className="w-[calc(100%-2rem)] max-w-md overflow-hidden p-0">  
        ...
        <div className="px-5 pb-5 pt-4">  
          ...
          <div className="mt-4 divide-y rounded-lg border bg-muted/30 text-sm">  
            <InfoRow icon={<User className="size-3.5" />} label="Người ký" value={hoTen} />  
            <InfoRow icon={<Award className="size-3.5" />} label="Cấp bậc" value={capBac} />  
            <InfoRow icon={<Briefcase className="size-3.5" />} label="Chức vụ" value={chucVu} />  
            <InfoRow icon={<Building2 className="size-3.5" />} label="Đơn vị" value={donViLabel} />  
            <InfoRow  
